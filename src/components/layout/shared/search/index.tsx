  ...
  return (
    <>
      {isBreakpointReached || settings.layout === 'horizontal' ? (
        ...
        <div className='flex items-center gap-2 cursor-pointer' onClick={() => setOpen(true)} suppressHydrationWarning>
          <IconButton className='text-textPrimary' onClick={() => setOpen(true)}>
            <i className='tabler-search text-2xl' />
          </IconButton>
          <div className='whitespace-nowrap select-none text-textDisabled' suppressHydrationWarning>
            Rechercher ⌘K
          </div>
        </div>
      )}
