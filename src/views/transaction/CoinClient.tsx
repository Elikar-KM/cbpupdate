...
const columnHelper = createColumnHelper<CoinTransaction>()
...
const CoinClient = () => {
  ...
  const [data, setData] = useState<CoinTransaction[]>([])
  ...
  const handleConfirm = async () => {
    ...
    try {
      await fetch(`http://localhost:8000/api/coin/${selectedTransaction.id}/confirm`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${session?.user?.accessToken}`
        }
  ...
  const handleReject = async () => {
    ...
    try {
      await fetch(`http://localhost:8000/api/coin/${selectedTransaction.id}/reject`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${session?.user?.accessToken}`
        }
  ...
  const columns = useMemo<ColumnDef<CoinTransaction, any>[]>(
    () => [
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        message={`Voulez-vous vraiment confirmer l'achat de ${selectedTransaction?.amount} $ en ${selectedTransaction?.crypto_type} ?`}
      ...
      <ConfirmModal
        ...
        message={`Voulez-vous vraiment rejeter cet achat ?`}
